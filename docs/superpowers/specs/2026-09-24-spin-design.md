...
- **Settling dark.** The worker accepts an external field with each tick (`sim.set_params(1, h)` when it changes). `h` is 0 at and above the critical point (2.269), so the critical clusters are unbiased; it ramps in over 2.27→2.0 to −0.015, holds while the domains are still mobile (2.0→1.4), then tapers to −0.005 at the cold end. Measured: a lattice jumped straight to the bottom drifts to a ground majority within about two seconds and keeps bone islands for tens of seconds; a gradual scroll (or the idle drift) reaches the bottom already dark and the last islands fade to black over the following seconds. Weaker fields left the cold end a coin toss on jumps; stronger ones erased the islands instantly. This is the compromise.
