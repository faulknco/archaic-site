...
- `public/forge/shared/gate.js` — the photosensitivity gate, lifted out of Spin: same Tab wrap, but each piece passes its own `sessionStorage` key (Spin `spin-gate`, Critical `critical-gate`), so accepting one page covers only that page for the browser session. (Changed 2026-09-25 from a shared key: a mild notice must never clear a flashing one.)
